...
class SeriesApi {
  ...
  Future<List<TvShowApiModel>> getShows() async {
    final response = await _dio.get('$_baseUrl/shows');
  ...
  Future<List<TvShowApiModel>> searchShows(String value) async {
    final response = await _dio.get('$_baseUrl/search/shows?q=$value');
  ...
  Future<List<EpisodeApiModel>> getEpisodes(String showId) async {
    final response = await _dio.get('$_baseUrl/shows/$showId/episodes');
